...
- **p-use (predicate-use):** the variable appears inside an `if` condition — its value determines which branch is taken.
- **c-use (computation-use):** the variable appears in a computation or return expression — its value contributes to a result but does not affect branching directly.
