...
<img src="media/statement_coverage_updated.PNG" alt="media/statement_coverage_updated.png"/>
...
- Found that EclEmma did not work with mock, and had to rewrite all test cases that used mock to get EclEmma working.Some aspects of the assignments are not intuitive enough to readily understand and thus can easily enable us to deviate from what's expected.

# 10 Comments/feedback on the lab itself

The assignment does a good job of connecting two concepts that are usually taught separately — coverage measurement and data flow analysis. Requiring both a tool-based report (EclEmma) and a manual DU-pair trace forces us to understand what the tool is actually measuring rather than just reading percentages. The intentional defects in the source code are a particularly strong design choice, as they teach us that failing tests and unreachable branches are diagnostic signals, not just failures.
